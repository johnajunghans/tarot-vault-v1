...
export default function SpreadCanvas({
    ...
    // Interaction state: dragging, marquee selection, and keyboard-assisted panning.
    ...
    const groupDragOrigins = useRef<Map<number, { x: number; y: number }>>(
        new Map()
    )
    ...
    // Visual stacking: derive base order from z-index, then raise selected and dragged cards.
    ...
    // Drag feedback: alignment guides for the active single-card drag case.
    ...
    const guides = useMemo<CanvasGuide[]>(() => {
        ...
        positions.forEach((pos, i) => {
            ...
            const otherRect = getRect(pos.x, pos.y, CARD_WIDTH, CARD_HEIGHT)

            for (const de of [draggedRect.left, draggedRect.right]) {
                for (const oe of [otherRect.left, otherRect.right]) {
                    if (de === oe) {
            ...
            for (const de of [draggedRect.top, draggedRect.bottom]) {
                for (const oe of [otherRect.top, otherRect.bottom]) {
                    if (de === oe) {
    ...
    const draggingRef = useRef<{ index: number; x: number; y: number } | null>(
        null
    )

    // Group drag lifecycle: track shared origins and mirror the lead card delta across selected cards.
        ...
        if (groupDragOrigins.current.size > 0 && dragStartPos.current) {
            ...
            if (current) {
                ...
                for (const [i, origin] of groupDragOrigins.current) {
                    ...
                    setValue(`positions.${i}.x`, clampedX, {
                        shouldDirty: true,
                    })
                    setValue(`positions.${i}.y`, clampedY, {
                        shouldDirty: true,
                    })
                }
            }
    ...
    // Marquee selection: window-level listeners own the drag rectangle and final selection set.
        ...
        const handleMouseUp = (e: MouseEvent) => {
            ...
            if (dist < 5) {
                ...
                positions.forEach((pos, i) => {
                    if (
                        rectsIntersect(
                            getRect(pos.x, pos.y, CARD_WIDTH, CARD_HEIGHT),
                            selectionRect
                        )
                    ) {
                        selected.add(i)
    ...
    // Space-pan: hold the modifier key, then drag the scroll container directly.
    ...
    // Zoom gestures: keep pointer-centered zoom stable across wheel and pinch inputs.
    ...
    useEffect(() => {
        ...
        const handleWheel = (e: WheelEvent) => {
            ...
            setZoomAtPoint(
                targetZoomRef.current * scaleFactor,
                e.clientX,
                e.clientY
            )
        }
    ...
    // Offscreen pointer derivation: convert off-canvas card positions into quiet edge arrows.
    ...
    }, [cards, positions, viewportState, zoom])
